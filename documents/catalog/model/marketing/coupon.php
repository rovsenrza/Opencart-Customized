...
class Coupon extends \Opencart\System\Engine\Model {
	...
	public function getCoupon(string $code): array {
		...
		if ($coupon_info && ($coupon_info['date_start'] == '0000-00-00' || $coupon_info['date_start'] <= $today) && ($coupon_info['date_end'] == '0000-00-00' || $coupon_info['date_end'] >= $today)) {
	...
	public function getCouponByCode(string $code): array {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE `code` = '" . $this->db->escape(trim($code)) . "' AND `status` = '1'");
