...
class Article extends \Opencart\System\Engine\Model {
	...
	public function getArticles(array $data = []): array {
		...
		$filter_search = trim((string)($data['filter_search'] ?? ''));
		$filter_tag = trim((string)($data['filter_tag'] ?? ''));

		if ($filter_search !== '' || $filter_tag !== '') {
			$conditions = [];

			if ($filter_search !== '') {
				$words = explode(' ', trim((string)preg_replace('/\s+/', ' ', $filter_search)));
				$words = array_filter($words);

				$search_parts = [];

				foreach ($words as $word) {
					$search_parts[] = "`ad`.`name` LIKE '" . $this->db->escape('%' . $word . '%') . "'";
				}
				...
				$conditions[] = "`ad`.`description` LIKE '" . $this->db->escape('%' . $filter_search . '%') . "'";

				$search_parts = [];

				foreach ($words as $word) {
					$search_parts[] = "`ad`.`tag` LIKE '" . $this->db->escape('%' . $word . '%') . "'";
				}
			...
			if ($filter_tag !== '') {
				$words = explode(' ', trim((string)preg_replace('/\s+/', ' ', $filter_tag)));
				$words = array_filter($words);

				$tag_parts = [];

				foreach ($words as $word) {
					$tag_parts[] = "`ad`.`tag` LIKE '" . $this->db->escape('%' . $word . '%') . "'";
				}

				if ($tag_parts) {
					$conditions[] = "(" . implode(" OR ", $tag_parts) . ")";
				}
			}
