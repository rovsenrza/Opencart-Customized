...
class Blog extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		$data['image'] = $this->getOriginalImageUrl($topic_info['image'] ?? '');
		...
		foreach ($results as $result) {
			...
			$image = $this->getOriginalImageUrl($result['image'] ?? '');
	...
	public function info(): ?\Opencart\System\Engine\Action {
		...
		if ($article_info) {
			...
			$data['image'] = $this->getOriginalImageUrl($article_info['image'] ?? '');
	...
	/**
	 * Build public URL for original uploaded image.
	 *
	 * @param string $image
	 *
	 * @return string
