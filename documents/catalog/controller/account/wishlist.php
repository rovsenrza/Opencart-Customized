...
class WishList extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		if ($this->customer->isLogged() && !$this->load->controller('account/login.validate')) {
	...
	public function list(): void {
		...
		if ($this->customer->isLogged()) {
			...
			$this->response->setOutput($this->getGuestList());
		}
	}
	...
	public function guest(): void {
		...
		if (isset($this->request->post['product_ids'])) {
			if (is_array($this->request->post['product_ids'])) {
				...
				$decoded = json_decode($this->request->post['product_ids'], true);
	 ...
	 */
	private function sanitizeProductIds(array $product_ids): array {
		$sanitized = [];
	...
	public function add(): void {
		...
		if (!$json) {
			...
			if ($this->customer->isLogged()) {
				...
				// Guest mode: allow wishlist add without forcing login.
				$json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'language=' . $this->config->get('config_language') . '&product_id=' . $product_id), $product_info['name'], $this->url->link('account/wishlist', 'language=' . $this->config->get('config_language')));
