...
class Search extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		if (isset($this->request->get['search'])) {
			$filter_search = trim((string)preg_replace('/\s+/', ' ', (string)$this->request->get['search']));
		} else {
			$filter_search = '';
		}

		if (isset($this->request->get['description'])) {
			$filter_description = (string)$this->request->get['description'];
		} else {
			$filter_description = '';
		}

		if (isset($this->request->get['tag'])) {
			$filter_tag = trim((string)preg_replace('/\s+/', ' ', (string)$this->request->get['tag']));
		} else {
			$filter_tag = '';
		}
		...
		if ($filter_search !== '') {
			$this->document->setTitle($this->language->get('heading_title') . ' - ' . $filter_search);
		} elseif ($filter_tag !== '') {
		...
		if ($filter_search !== '') {
			$url .= '&search=' . urlencode(html_entity_decode($filter_search, ENT_QUOTES, 'UTF-8'));
		}
		...
		if ($filter_tag !== '') {
			$url .= '&tag=' . urlencode(html_entity_decode($filter_tag, ENT_QUOTES, 'UTF-8'));
		}
		...
		if ($filter_search !== '') {
			$data['heading_title'] = $this->language->get('heading_title') . ' - ' . $filter_search;
		} else {
