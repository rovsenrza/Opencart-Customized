...
class SeoUrl extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		if (isset($this->request->get['filter_store_id']) && $this->request->get['filter_store_id'] !== '') {
			$filter_store_id = (int)$this->request->get['filter_store_id'];
		} else {
			$filter_store_id = '';
		}

		if (isset($this->request->get['filter_language_id']) && $this->request->get['filter_language_id'] !== '') {
	...
	public function refresh(): void {
		...
		if (!$json) {
			// Endpoint intentionally kept as a no-op to avoid runtime failures from legacy broken logic.
			$json['success'] = $this->language->get('text_success');
		}
