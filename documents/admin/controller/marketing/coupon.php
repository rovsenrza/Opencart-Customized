...
class Coupon extends \Opencart\System\Engine\Controller {
	...
	public function save(): void {
		...
		$post_info['code'] = trim((string)$post_info['code']);
		$post_info['type'] = (string)$post_info['type'];
		$post_info['date_start'] = trim((string)$post_info['date_start']);
		$post_info['date_end'] = trim((string)$post_info['date_end']);
		$post_info['discount'] = (float)$post_info['discount'];
		$post_info['total'] = (float)$post_info['total'];
		$post_info['uses_total'] = (int)$post_info['uses_total'];
		$post_info['uses_customer'] = (int)$post_info['uses_customer'];
		...
		if (!in_array($post_info['type'], ['P', 'F'], true)) {
			$json['error']['warning'] = $this->language->get('error_type');
		}

		if ($post_info['discount'] < 0 || $post_info['total'] < 0 || $post_info['uses_total'] < 0 || $post_info['uses_customer'] < 0) {
			$json['error']['warning'] = $this->language->get('error_value');
		}

		if (!$this->isValidDate($post_info['date_start']) || !$this->isValidDate($post_info['date_end'])) {
			$json['error']['warning'] = $this->language->get('error_date');
		}

		if ($this->isValidDate($post_info['date_start']) && $this->isValidDate($post_info['date_end']) && $post_info['date_start'] && $post_info['date_end'] && $post_info['date_start'] > $post_info['date_end']) {
			$json['error']['warning'] = $this->language->get('error_date');
		}
